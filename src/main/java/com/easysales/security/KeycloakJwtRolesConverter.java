//package com.easysales.security;
//
...
public class KeycloakJwtRolesConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
    ...
    public Collection<GrantedAuthority> convert(Jwt jwt) {
        ...
        // Resource (client) roles
        Map<String, Map<String, Object>> resourceAccess = jwt.getClaim(CLAIM_RESOURCE_ACCESS);

        if (resourceAccess != null) {
            resourceAccess.forEach((resource, resourceClaims) -> {
                if (resourceClaims.containsKey(CLAIM_ROLES)) {
                    Collection<String> roles = (Collection<String>) resourceClaims.get(CLAIM_ROLES);
                    if (roles != null && !roles.isEmpty()) {
                        roles.forEach(
                                role -> grantedAuthorities.add(new SimpleGrantedAuthority(PREFIX_RESOURCE_ROLE + resource + "_" + role))
                        );
